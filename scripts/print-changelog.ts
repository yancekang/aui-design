...
const QUERY_AUTHOR =
  '.pull-discussion-timeline .TimelineItem:first .author:first';
...
async function printLog() {
  ...
  let { toVersion } = await inquirer.prompt([
    {
      type: 'list',
      name: 'toVersion',
      message: `🔀 Please choose branch to compare with ${chalk.magenta(
        fromVersion,
      )}:`,
      choices: [
        'master',
  ...
  if (!/\d+\.\d+\.\d+/.test(fromVersion)) {
    console.log(
      chalk.red(`🤪 tag (${chalk.magenta(fromVersion)}) is not valid.`),
    );
  }
  ...
  for (let i = 0; i < logs.all.length; i += 1) {
    ...
    for (let j = 0; j < prs.length; j += 1) {
      ...
      const english = getDescription(
        lines.find((line) => line.text.includes('🇺🇸 English')),
      );
      const chinese = getDescription(
        lines.find((line) => line.text.includes('🇨🇳 Chinese')),
      );
  ...
  let html = fs.readFileSync(
    path.join(__dirname, 'previewEditor', 'template.html'),
    'utf8',
  );
  html = html.replace(
    '// [Replacement]',
    `window.changelog = ${JSON.stringify(prList)};`,
  );
  fs.writeFileSync(
    path.join(__dirname, 'previewEditor', 'index.html'),
    html,
    'utf8',
  );
